...
const { repairTurbopackCache } = require('./turbopack-cache');
const {
  OAX_WEB,
  ensureWebEnvFile,
  sanitizeProfileName,
...
const WEB_STOP_ESCALATION_MS = 5000;
...
async function startPod() {
  ...
  repairTurbopackCache(TURBOPACK_CACHE_DIR);
...
function stopPod() {
  return new Promise((resolve) => {
    ...
    for (const p of state.procs) {
      ...
      const initialSignal = p.name === 'web' ? 'SIGINT' : 'SIGTERM';
      initialSignals.set(p.pid, initialSignal);
      killTree(p.pid, initialSignal);
      console.log(`✓ Sent ${initialSignal} to ${p.name} (pid ${p.pid})`);
    }

    // Give them time to exit cleanly before SIGKILL.
    ...
    const sweep = () => {
      ...
      for (const p of stillAlive) {
        ...
        if (initialSignals.get(p.pid) !== 'SIGINT') continue;
        if (Date.now() - stopStartedAt < WEB_STOP_ESCALATION_MS) continue;
        killTree(p.pid, 'SIGTERM');
        initialSignals.set(p.pid, 'SIGTERM');
        console.log(`⚠️  Escalating ${p.name} (pid ${p.pid}) to SIGTERM`);
      }
