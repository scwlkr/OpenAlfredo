...
// DOP API security middleware.
// Requires a Bearer token or X-DOP-Key header on all /api/* routes.
// The key is loaded from the DOP_API_KEY environment variable, which is
...
export function middleware(request: NextRequest) {
  ...
  const apiKey = process.env.DOP_API_KEY;
  if (!apiKey) {
  ...
  // Also accept X-DOP-Key header (for simpler curl usage)
  const headerKey = request.headers.get('x-dop-key');
